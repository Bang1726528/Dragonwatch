...
local scanWorldBosses = function()
	for _k, name in WORLD_BOSSES do
		Dragonwatch_targets[name] = true
	end
end

local Dragonwatch = CreateFrame'Frame'
Dragonwatch:SetScript('OnUpdate', function() Dragonwatch.UPDATE() end)
Dragonwatch:SetScript('OnEvent', function() Dragonwatch.LOAD(); scanWorldBosses() end)
Dragonwatch:RegisterEvent'VARIABLES_LOADED'
...
do
	...
	function Dragonwatch.play_sound()
		if not last_played or GetTime() - last_played > 10 then -- 8
			SetCVar('MasterSoundEffects', 0)
			SetCVar('MasterSoundEffects', 1)
			PlaySoundFile[[Interface\AddOns\Dragonwatch\Event_wardrum_ogre.ogg]]
			PlaySoundFile[[Interface\AddOns\Dragonwatch\scourge_horn.ogg]]
			last_played = GetTime()
		end
	end
end

function Dragonwatch.check_for_targets()
	for name, _ in Dragonwatch_targets do
		if Dragonwatch.target(name) and not UnitIsPlayer("target") then
			Dragonwatch.toggle_target(name)
			Dragonwatch.play_sound()
			Dragonwatch.flash.animation:Play()
			Dragonwatch.button:set_target()
		end
	end
end
...
do
	...
	function Dragonwatch.target(name)
...
function Dragonwatch.LOAD()
	do
		local flash = CreateFrame'Frame'
		Dragonwatch.flash = flash
		flash:Show()
		flash:SetAllPoints()
		flash:SetAlpha(0)
	...
	local button = CreateFrame('Button', 'Dragonwatch_button', UIParent)
	button:Hide()
	Dragonwatch.button = button
	...
	button:SetNormalTexture[[Interface\AddOns\Dragonwatch\UI-Achievement-Parchment-Horizontal]]
	button:SetBackdrop{
		tile = true,
		edgeSize = 16,
	...
	do
		...
		title_background:SetTexture[[Interface\AddOns\Dragonwatch\UI-Achievement-Title]]
	...
	do
		...
		glow:SetTexture[[Interface\AddOns\Dragonwatch\UI-Achievement-Alert-Glow]]
	...
	do
		...
		shine:SetTexture[[Interface\AddOns\Dragonwatch\UI-Achievement-Alert-Glow]]
...
do
	Dragonwatch.last_check = GetTime()
	function Dragonwatch.UPDATE()
		if GetTime() - Dragonwatch.last_check >= CHECK_INTERVAL then
			Dragonwatch.last_check = GetTime()
			Dragonwatch.check_for_targets()
		end
	end
end

function Dragonwatch.print(msg)
	if DEFAULT_CHAT_FRAME then
		DEFAULT_CHAT_FRAME:AddMessage(LIGHTYELLOW_FONT_COLOR_CODE .. '<Dragonwatch> ' .. msg)
	end
end

function Dragonwatch.sorted_targets()
	local sorted_targets = {}
	for key in pairs(Dragonwatch_targets) do
...
function Dragonwatch.toggle_target(name)
	local key = strupper(name)
	if Dragonwatch_targets[key] then
		Dragonwatch_targets[key] = nil
		Dragonwatch.print('- ' .. key)
	elseif key ~= '' then
		Dragonwatch_targets[key] = true
		Dragonwatch.print('+ ' .. key)
	end
end
	
SLASH_Dragonwatch1 = '/Dragonwatch'
function SlashCmdList.Dragonwatch(parameter)
	local _, _, name = strfind(parameter, '^%s*(.-)%s*$')
	
	if name == '' then
		for _, key in ipairs(Dragonwatch.sorted_targets()) do
			Dragonwatch.print(key)
		end
	else
		Dragonwatch.toggle_target(name)
